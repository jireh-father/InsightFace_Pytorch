...
image_root = '/media/irelin/data_disk/dataset/afp/noseprint_recognition/595_dogs_nose'
output_dir = '/media/irelin/data_disk/dataset/afp/noseprint_recognition/nose_split_choice'

annos = json.load(open(anno_file))

for i, anno in enumerate(annos):
    im_fn = os.path.basename(anno['data']['image']).split('-')[1]
    found = glob.glob(os.path.join(image_root, "*" + im_fn))
    if not found:
        print("not exists")
        continue
    im_path = found[0]
